...
int try_s(char **args, char **message, int n_args) {
    ...
    int res = check_if_in_time(PLID);
    if (res == 2) { // 2 if not in time
        strcpy(status, "EMT");
        fprintf(stderr, "TEMPO ULTRPASSADO L !!!\n");
        //if (end_game_after_try())
        //    return 1;
        //if (send_end_try_message(OP_CODE, status, message)) // manda secret key
        //    return 1;
        //else return 0;
    } else if (res == 0) {
        //if (try_game())
        //    return 1;
        // send_try_message() // manda aqueles n
...
int check_ongoing_game(const char PLID[PLID_SIZE + 1]) {
    ...
    int ret_value;
    if (access(filename, F_OK) == 0) ret_value = 1; // ongoing game
    else ret_value = 0;
...
int check_if_in_time(char PLID[7]) {
    ...
    if (n == -1) {
        fprintf(stderr, "Error: failed to read %s file.\n", filename);
        free(path);
        close(fd);
        return 1;
    }
